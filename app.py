...
# --- 2. GŁÓWNY CSS (Powrót do luksusowej szaty graficznej) ---
st.markdown("""
<style>
    .stApp {
...
# ==========================================
# 3. BAZA DANYCH (LOGIKA: TEST ZGODNOŚCI)
# ==========================================

toasty = [
    "Wypijcie zdrowy łyk za Waszą namiętność! 🥂",
    "Toast za najodważniejszą osobę w tym pokoju! 🔥",
    "Pijemy za Waszą wspólną przyszłość i dzisiejszą noc! 😈"
]
...
kary_l1 = ["Całuj moją szyję przez minutę.", "Zrób mi masaż dłoni.", "Zdejmij skarpetki."]
kary_l2 = ["Zdejmij z siebie jedną rzecz.", "Pocałuj moje wewnętrzne udo.", "Zrób łyka alkoholu i przekaż mi go ustami."]
kary_l3 = ["Zliż kroplę alkoholu z mojego brzucha.", "Zostań tylko w bieliźnie.", "Wymasuj moje pośladki."]
kary_l4 = ["Zaspokajaj mnie ustami przez minutę.", "Rób ze mną co chcesz przez 2 minuty.", "Zdejmij wszystko."]
...
# --- 4. SYNCHRONIZACJA STANU (GLOBALNA) ---
@st.cache_resource
    ...
    if q_idx < len(state["gra"]):
        ...
        if state["status"] == "question":
            ...
            <div class='premium-box' style='background:{bg};'>
                ...
                <p style='color: #8c7a96; font-size: 18px;'>Zasada: Możesz wykupić się shotem! 🥃</p>
            </div>
            """, unsafe_allow_html=True)
            time.sleep(6) # 6 sekund na karę i auto-przejście
            state["current_q"] += 1
            state["status"] = "question"
            st.rerun()
    else:
        st.markdown("<div class='premium-box'><h1 class='gold-text'>KONIEC GRY.<br>Czas na Was. 😈</h1></div>", unsafe_allow_html=True)
...
# PILOT
    ...
    if q_idx < len(state["gra"]):
        ...
        if q["kto"] == "TOAST":
            ...
            # Informacja kogo oceniamy (właściciela pytania)
